...
{  
    ...
    if (GlyphMappers.Latin.canHandleEncoding(ename))
        return new GlyphMappers.Latin(fontDict, srcfile);
    else if (GlyphMappers.Indentity.canHandleEncoding(ename))
        return new GlyphMappers.Indentity(fontDict, srcfile);
...
static Map readGlyphList(String name)
{
    ...
    try {
        if(s==null)
            throw new IOException("Internal error: couldn't locate resouce " + name);
        ...
        parser.commentChar('#');
        // let us know about lineends, so we can get glyphs that map to more than one code
