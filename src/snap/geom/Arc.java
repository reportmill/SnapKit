...
public class Arc extends RectBase {
    ...
    {
        Shape path = getPath();
        return path.getPathIter(aTrans);
    }
     ...
     */
    @Override
    public Arc copyForBounds(double aX, double aY, double aW, double aH)
    {
        return new Arc(aX, aY, aW, aH, _start, _sweep, _holeRatio);
    }
    ...
    private Path2D getPath()
    {
        ...
        double arcHalfW = bounds.width / 2;
        double arcHalfH = bounds.height / 2;
        double midX = bounds.getMidX();
        ...
        double y1 = midY + sin(startAngle) * arcHalfH;
        ...
        if (getClosure() == Closure.Pie && sweep < 360) {
            double x0 = midX + cos(startAngle) * arcHalfW * holeRatio;
            double y0 = midY + sin(startAngle) * arcHalfH * holeRatio;
            path.moveTo(x0, y0);
            path.lineTo(x1, y1);
        }
        ...
        // Append sweep
        appendArc(path, midX, midY, arcHalfW, arcHalfH, startAngle, sweep);

        // If HoleRatio, append reverse sweep
        if (holeRatio > 0) {

            // Append line to inner arc (or close/move if full circle)
            double x = midX + cos(endAngle) * arcHalfW * holeRatio;
            double y = midY + sin(endAngle) * arcHalfH * holeRatio;
            if (sweep < 360)
                path.lineTo(x, y);
            else {
                path.close();
            ...
            appendArc(path, midX, midY, arcHalfW * holeRatio, arcHalfH * holeRatio, endAngle, -sweep);
        }
    ...
    private static void appendArc(Path2D aPath, double midX, double midY, double hw, double hh, double angle, double aSweep)
    {
        ...
        double sweepRemainder = sweep == 90 || sweep == -90 ? aSweep - sweep : 0;
        ...
        if (sweepRemainder != 0)
            appendArc(aPath, midX, midY, hw, hh, angle + sweep, sweepRemainder);
    }
    ...
    public boolean equals(Object anObj)
    {
        ...
        Arc other = anObj instanceof Arc ? (Arc) anObj : null;
