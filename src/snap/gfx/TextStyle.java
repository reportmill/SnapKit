 ...
 */
public TextStyle copyFor(Object anObj) { String key = getStyleKey(anObj); return key!=null? copyFor(key,anObj) : this; }
...
public TextStyle copyFor(Object ... theObjs)
{
    ...
    for(Object obj : theObjs) { String key = getStyleKey(obj); if(key!=null) clone.setValue(key, obj); }
    return clone;
}
...
{
    if(aKey.equals(FONT_KEY)) _font = (Font)aValue;
    else if(aKey.equals(COLOR_KEY)) _color = (Color)aValue;
    else if(aKey.equals(UNDERLINE_KEY)) _underline = SnapUtils.intValue(aValue);
    else if(aKey.equals(SCRIPTING_KEY)) _scripting = SnapUtils.intValue(aValue);
    else if(aKey.equals(CHAR_SPACING_KEY)) _charSpacing = SnapUtils.doubleValue(aValue);
    else if(aKey.equals(LINK_KEY)) _link = (TextLink)aValue;
    else if(aKey.equals(FORMAT_KEY)) _format = (TextFormat)aValue;
    else if(aKey.equals(BORDER_KEY)) _border = (Border)aValue;
