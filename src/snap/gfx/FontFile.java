...
public abstract class FontFile {
    ...
    FontFile              _boldVersion = null;
    
    // Cached reference to italic version of font
    FontFile              _italicVersion = null;
...
{
    FontFile ffile = (FontFile)_allFontFiles.get(aName);
    if(ffile==null) _allFontFiles.put(aName, ffile = GFXEnv.getEnv().getFontFile(aName));
    return ffile;
}
...
public double charAdvance(char aChar)
{
    ...
    if(aChar<_charWidths.length) {
        ...
        if(cwidth<0) cwidth = _charWidths[aChar] = (float)charAdvanceImpl(aChar);
        return cwidth;
    }
