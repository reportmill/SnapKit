 ...
 * Creates a new ImagePaint.
 */
public ImagePaint(Image anImage)  { this(anImage, 0, 0, anImage.getWidth(), anImage.getHeight(), true); }

/**
 * Creates a new ImagePaint.
 */
public ImagePaint(Image anImage, Rect aRect)  { this(anImage, aRect, true); }
...
{
    _image = anImage; _x = aX; _y = aY; _w = aW; _h = aH; _abs = isAbs;
}
...
public XMLElement toXML(XMLArchiver anArchiver)
{
    ...
    // Archive Tile
    if(!isAbsolute()) e.add("Tile", false);
    
    // Archive bounds
    if(_x!=0) e.add("x", _x); if(_y!=0) e.add("y", _y);
    if(_w!=_image.getWidth()) e.add("w", _w); if(_h!=_image.getHeight()) e.add("h", _h);
...
public Object fromXML(XMLArchiver anArchiver, XMLElement anElement)
{
    ...
    if(iname!=null) {
        ...
        _w = _image.getWidth(); _h = _image.getHeight();
    }
    ...
    // Unarchive bounds
    if(anElement.hasAttribute("x")) _x = anElement.getAttributeFloatValue("x");
    if(anElement.hasAttribute("y")) _y = anElement.getAttributeFloatValue("y");
    if(anElement.hasAttribute("w")) _w = anElement.getAttributeFloatValue("w");
    if(anElement.hasAttribute("h")) _h = anElement.getAttributeFloatValue("h");
    ...
    if(sx!=1 || sy!=1) { _w = _abs? _w*sx : sx; _h = _abs? _h*sy : sy; }
