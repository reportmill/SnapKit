...
public class GradientPaint implements Paint, Cloneable, XMLArchiver.Archivable {
    ...
    double      _sx, _sy = .5, _ex = 1, _ey = .5;
    
    // The roll (if linear and not absolute)
    double      _roll;
    
    // The stops
    Stop        _stops[] = DEFAULT_STOPS;
    ...
    // Default stops
    static Stop    DEFAULT_STOPS[] = new Stop[] { new Stop(0,Color.BLACK), new Stop(1,Color.WHITE) };
 ...
 */
public GradientPaint(Type aType, Stop theStops[])  { this(aType, aType==Type.LINEAR? 0 : .5, .5, 1d, .5, theStops); }
...
{
    _type = aType; _sx = aSX; _sy = aSY; _ex = aEX; _ey = aEY; _stops = theStops; _abs = isAbs;
}
...
protected void setRoll(double aRoll, Rect aRect)
{
    ...
    // Handle common cases
    if(aRoll==0) { _sx = aRect.getX(); _sy = aRect.getMidY(); _ex = aRect.getMaxX(); _ey = aRect.getMidY(); return; }
    ...
    Rect r2 = t.createTransformedShape(aRect).getBounds();                  // Get bounds of transformed rect
...
public boolean equals(Object anObj)
{
    ...
    if(_sx!=other._sx || _sy!=other._sy || _ex!=other._ex || _ey!=other._ey) return false; 
...
public XMLElement toXML(XMLArchiver anArchiver)
{
    ...
    // Archive Points/Roll
    if(isRadial()) {
        e.add("x0", _sx); e.add("y0", _sy); e.add("x1", _ex); e.add("y1", _ey); }
    else if(getRoll()!=0) e.add("roll", _roll);
    
    // Archive first color        
    if(!getStopColor(0).equals(Color.BLACK)) e.add("color", "#" + getStopColor(0).toHexString());
    ...
    for(int i=0, iMax=getStopCount(); i<iMax; ++i) {
        ...
        if((i==0 && MathUtils.equalsZero(offset)) || (i==iMax-1 && MathUtils.equals(offset, 1))) continue;
    ...
    if(getStopCount()!=2) e.add("nstops", getStopCount());
...
public static Stop[] getStops(double theOffs[], Color ... theColors)
{
    ...
    for(int i=0;i<theOffs.length;i++) stops[i] = new Stop(theOffs[i], theColors[i]);
    return stops;
}
