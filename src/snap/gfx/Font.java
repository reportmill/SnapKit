 ...
 */
public Font()  { this(Arial12._fontFile, 12); }

/**
 * Creates a font for the given font file and point size.
 */
protected Font(FontFile aFontFile, double aPointSize)  { _fontFile = aFontFile; _size = aPointSize; }
...
public Rect getCharBounds(char aChar)
{
    ...
    Rect cbounds = path.getBounds(); cbounds.scale(_size);
    return cbounds;
}
 ...
 */
public Font getBold()  { FontFile ff = _fontFile.getBold(); return ff!=null? new Font(ff, _size) : null; }

/**
 * Returns the italic version of this font.
 */
public Font getItalic()  { FontFile ff=_fontFile.getItalic(); return ff!=null? new Font(ff, _size) : null; }

/**
 * Returns a font with the same family as the receiver but with the given size.
 */
public Font deriveFont(double aPointSize) { return aPointSize==_size? this : new Font(_fontFile, aPointSize); }

/**
 * Returns a font with the same family as the receiver but with size adjusted by given scale factor.
 */
public Font scaleFont(double aScale)  { return aScale==1? this : new Font(_fontFile, _size*aScale); }
 ...
 */
public Object getNative()  { return _native!=null? _native : (_native=_fontFile.getNative(_size)); }
 ...
 */
public static String[] getFontNames(String aFamilyName)  { return GFXEnv.getEnv().getFontNames(aFamilyName); }
