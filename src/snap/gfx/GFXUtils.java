...
public static void convolve(int spix[], int dpix[], int sw, int sh, float kern[], int kw)
{
    ...
    IntStream.range(0,pcount).parallel().forEach(i -> {
        ...
        int yMin = dy + (sh-2*dy)/pcount*i, yMax = dy + (sh-2*dy)/pcount*(i+1); if(i==pcount-1) yMax = sh-dy;
        int off = yMin*sw + dx; // If single thread: yMin=dy,yMax=sh-dy
    
        // Iterate over pix map
        for(int y=yMin;y<yMax;y++,off+=dx*2) for(int x=dx,xMax=sw-dx;x<xMax;x++,off++)
...
public static float[] getGaussianKernel(int rx, int ry)
{
    ...
    int kw = rx*2 + 1, kh = ry*2 + 1, klen = kw*kh; float kern[] = new float[klen];
    double devx = rx>0? rx/3d : .1, devy = ry>0? ry/3d : .1; // This guarantees non zero values in the kernel
    double devxSqr2 = 2*devx*devx, devySqr2 = 2*devy*devy, devSqr2Pi = Math.max(devxSqr2,devySqr2)*Math.PI;
    double sum = 0;
