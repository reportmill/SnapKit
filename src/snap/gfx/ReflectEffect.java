...
public Image getReflectImage(PainterDVR aPDVR, Rect aRect)
{
    ...
    double refHeight = getReflectHeight(), fadeHeight = getFadeHeight();
    int width = (int)Math.round(aRect.getWidth()), height = (int)Math.ceil(aRect.getHeight()*refHeight*fadeHeight);
    
    // Get original graphics as flipped image
    Image img = Image.get(width, height, true);
    Painter pntr = img.getPainter(); pntr.setImageQuality(1); pntr.clipRect(0, 0, width, height);
    pntr.save();
    pntr.scale(1,-refHeight); pntr.translate(-aRect.getX(), -aRect.getMaxY());
    aPDVR.exec(pntr);
    pntr.restore();
    
    // Create gradient paint to fade image out
    Color c1 = new Color(1d,.5), c2 = new Color(1d, 0d);
    ...
    pntr.setPaint(mask); pntr.fillRect(0, 0, width, height);
    return img;
}
 ...
 */
public ReflectEffect copyForReflectHeight(double aRH)  { return new ReflectEffect(aRH, _fadeHeight, _gap); }

/**
 * Returns a reflection for given fade height.
 */
public ReflectEffect copyForFadeHeight(double aFH)  { return new ReflectEffect(_refHeight, aFH, _gap); }
