 ...
 */
public class ColorPanel extends SNPViewController {
    ...
    {
        return _shared != null ? _shared : (_shared = new ColorPanel());
    }
    ...
    {
        return getViewIntValue("PickerPanel");
    }
     ...
     */
    private String toHexString(Color aColor)
    {
        return '#' + aColor.toHexString();
    }

    private Color fromHexString(String aHS)
    {
        Color c = Color.get(aHS);
    ...
    public static class ImagePicker extends View {
        ...
        {
            int x = (int) Math.round(getWidth() - _image.getWidth()) / 2;
            if (x < 0) x = 0;
            int y = (int) Math.round(getHeight() - _image.getHeight()) / 2;
            if (y < 0) y = 0;
            aPntr.setColor(Color.WHITE);
            aPntr.fillRect(0, 0, getWidth(), getHeight());
            aPntr.drawImage(_image, x, y);
        }
