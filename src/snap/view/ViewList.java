 ...
 */
protected void add(View aView)  { add(aView, size()); }
...
{
    _views = ArrayUtils.add(_views, aView, anIndex); _managed = null;
}
...
protected View remove(int anIndex)
{
    ...
    _views = ArrayUtils.remove(_views, anIndex); _managed = null;
    return child;
}
 ...
 */
protected void removeAll()  { for(int i=size()-1; i>=0; i--) remove(i); }

/**
 * Sets views to given list.
 */
protected void setAll(View ... theViews)  { removeAll(); for(View c : theViews) add(c); }
...
public View getView(String aName)
{
    for(View view : _views) {
        if(aName.equals(view.getName())) return view;
...
/**
 * Returns the views of given class (optional) hit by given shape, excluding given view (optional).
 */
public <T extends View> List <T> getViewsAt(Shape aShape, Class <T> aClass, View aView)
{
...
{
    Rect bnds = aView.getBoundsLocal(); if(anInset!=0) bnds.inset(anInset);
    Shape shp = aView.localToParent(bnds);
    return getViewAt(shp, aClass, aView);
}

/**
 * Returns the views of given class (optional) intersecting given view.
 */
public <T extends View> List <T> getHitViews(View aView, Class <T> aClass, double anInset)
{
    Rect bnds = aView.getBoundsLocal(); if(anInset!=0) bnds.inset(anInset);
    Shape shp = aView.localToParent(bnds);
    return getViewsAt(shp, aClass, aView);
}
