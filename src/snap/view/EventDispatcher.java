...
public class EventDispatcher {
    ...
    public void dispatchMouseEvent(ViewEvent anEvent)
    {
        ...
        for (View view : targParents) {
            if (view.getEventAdapter().isFilterTypeEnabled(anEvent.getType())) {
        ...
        for (int i = targParents.length - 1; i >= 0; i--) {
            ...
            if (view.getEventAdapter().isHandlerTypeEnabled(anEvent.getType())) {
                ViewEvent e2 = anEvent.copyForView(view);
    ...
    private View getTargetView(ViewEvent anEvent, View deepView)
    {
        ...
        switch (anEvent.getType()) {
            ...
            // Can't happen
            default:
                System.err.println("EventDispatcher.getTarget: Unknown type: " + anEvent.getType());
                return null;
        }
    }
    ...
    private void updateMouseOvers(View[] deepParents, ViewEvent anEvent)
    {
        ...
        for (int i = _mouseOvers.size() - 1; i >= 0; i--) {
            View view = _mouseOvers.get(i);
            ...
            if (!ArrayUtils.containsId(deepParents, view)) {
                ...
                if (!view.getEventAdapter().isTypeEnabled(MouseExit))
                    continue;
        ...
        for (int i = _mouseOvers.size(); i < deepParents.length; i++) {
            View view = deepParents[i];
            _mouseOvers.add(view);
            _mouseOverView = view;
            if (!view.getEventAdapter().isTypeEnabled(MouseEnter))
                continue;
    ...
    public void dispatchDragTargetEvent(ViewEvent anEvent)
    {
        ...
        View rootView = _win.getRootView();
        View targ = ViewUtils.getDeepestViewAt(rootView, anEvent.getX(), anEvent.getY());
        ...
        int start = getParentCount(_dragOverView != null ? _dragOverView : rootView);
    ...
    private int getParentCount(View aView)
    {
        ...
        int pc = 1;
        for (View v = aView; v != rview; v = v.getParent()) pc++;
        return pc;
    }
    ...
    private View[] getParents(View aView)
    {
        ...
        View[] pars = new View[pc];
        if (pc == 0) return pars;
        View rview = _win.getRootView();
        for (View v = aView; v != rview; v = v.getParent()) pars[--pc] = v;
        pars[0] = rview;
        return pars;
    }
    ...
    {
        return ViewUtils.getMouseDown().getClickCount();
    }
}