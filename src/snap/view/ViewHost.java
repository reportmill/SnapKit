...
public static View[] getGuests(ViewHost aHost)
{
    ...
    for(int i=0;i<gc;i++) guests[i] = aHost.getGuest(i);
    return guests;
}
...
public static int indexOfGuest(ViewHost aHost, View aView)
{
    ...
    for(int i=0,iMax=aHost.getGuestCount();i<iMax;i++) { View guest = aHost.getGuest(i);
        if(guest==aView) return i; }
    return -1;
}
...
{
    // Archive guests
    for(int i=0, iMax=aHost.getGuestCount(); i<iMax; i++) { View child = aHost.getGuest(i);
