...
public void addPropChangeListener(PropChangeListener aLsnr)
{
    ...
    if(hasListener(aLsnr,null)) { System.err.println("PropChangeSupport.add: Adding duplicate listener"); return; }
    
    // If none, just set, otherwise forward to nested PCS
    if(_pcl==null)
        _pcl = aLsnr;
    else {
        if(_pcs==null) _pcs = new PropChangeSupport(_src);
...
{
    // Do check for given listener
    if(hasListener(aLsnr,aProp)) { System.err.println("PropChangeSupport.add: Adding duplicate listener"); return; }
    
    // If none, just set, otherwise forward to nested PCS
    if(_pcl==null) {
        _pcl = aLsnr; _pclProp = aProp; }
    else {
        if(_pcs==null) _pcs = new PropChangeSupport(_src);
 ...
 */
public boolean hasDeepListener()  { return _dcl!=null || (_pcs!=null && _pcs.hasDeepListener()); }
