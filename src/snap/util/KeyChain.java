...
public class KeyChain {
    ...
    // The KeyChain parser
    static KeyChainParser _parser = new KeyChainParser();

    // A shared map of previously encountered key chains
    static Map <Object,KeyChain>  _keyChains = new Hashtable();
 ...
 * Node constructor.
 */
public KeyChain(Op anOp)  { _op = anOp; }

/**
 * Node constructor.
 */
public KeyChain(Op anOp, Object child)  { _op = anOp; addChild(child); }

/**
 * Node constructor.
 */
public KeyChain(Op anOp, Object left, Object right)  { _op = anOp; addChild(left); addChild(right); }

/**
 * Node constructor.
 */
public KeyChain(Object cond, Object tExp, Object fExp)
{
    _op = Op.Conditional;
...
public KeyChain subchain(int anIndex)
{
    ...
    KeyChain kc = new KeyChain(Op.Chain); for(int i=anIndex; i<ccount; i++) kc.addChild(getChild(i));
    return kc;
}
 ...
 * Returns the result of evaluating the given key chain on the given object.
 */
public static Object getValue(Object anObj, KeyChain aKeyChain)  { return getValue(anObj, anObj, aKeyChain); }
...
public static void setValue(Object anObj, KeyChain aKeyChain, Object aValue)
{
    ...
    if(kchain.getOp()==Op.Chain) { int cc = aKeyChain.getChildCount();
        KeyChain kc = new KeyChain(Op.Chain); for(int i=0;i<cc-1;i++) kc.addChild(aKeyChain.getChild(i));
