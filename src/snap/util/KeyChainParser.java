...
public static class StatementHandler extends ParseHandler <KeyChain> {
    ...
    public void parsedOne(ParseNode aNode, String anId)
    {
        ...
        // Handle Expression
            ...
            else if(_op==1) _part = new KeyChain(Op.Assignment, _part, expr);
...
public static class ExpressionHandler extends ParseHandler <KeyChain> {
    ...
    {
        // Handle KeyChain
        if(aNode.getCustomNode() instanceof KeyChain) { KeyChain kc = aNode.getCustomNode(KeyChain.class);
            if(_part==null) _part = kc;
            else if(_part.getOp()!=Op.Conditional) _part = new KeyChain(Op.Conditional, _part, kc);
...
public static class UnaryExprHandler extends ParseHandler <KeyChain> {
    ...
    public void parsedOne(ParseNode aNode, String anId)
    {
        ...
        if(anId=="KeyChain") {
            KeyChain kc = aNode.getCustomNode(KeyChain.class);
            _part = _op==null? kc : new KeyChain(_op, kc); _op = null;
        }
...
public static class ArgListHandler extends ParseHandler <KeyChain> {
    ...
    public void parsedOne(ParseNode aNode, String anId)
    {
        ...
        if(anId=="Expression") {
            ...
            if(_part==null) _part = new KeyChain(Op.ArgList, arg);
