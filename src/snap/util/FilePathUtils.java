...
{
    String parent = getParent(aPath); if(parent.length()==0) parent = "/";
...
{
    String path = getSimple(aPath); if(!anExtension.startsWith(".")) path += ".";
...
public static String[] getNativePaths(String thePaths[])
{
    ...
    for(int i=0;i<thePaths.length;i++) npaths[i] = getNativePath(thePaths[i]);
    return npaths;
}
