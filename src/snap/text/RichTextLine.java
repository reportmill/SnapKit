...
public class RichTextLine implements CharSequence, Cloneable {
    ...
    protected List<RichTextRun>  _runs = new ArrayList();

    // The line style
    private TextLineStyle  _lstyle;
    ...
    public int indexOf(String aStr, int aStart)
    {
        for (RichTextRun run : getRuns()) {
            if (aStart >= run.getEnd()) continue;
            int rstrt = run.getStart();
            int index = run._sb.indexOf(aStr, aStart - rstrt);
            if (index >= 0) return index + rstrt;
        }
        return -1;
    }
    ...
    public void removeChars(int aStart, int anEnd)
    {
        ...
        while (aStart < end) {
            ...
            if (end == run.getStart()) run = getRun(run.getIndex() - 1);
            int rstart = run.getStart(), start = Math.max(aStart, rstart);

            // If range matches run range, just remove it
            if (start == rstart && end == run.getEnd() && getRunCount() > 1) {
                int rindex = run.getIndex();
                _runs.remove(rindex);
                updateRuns(rindex - 1);
            }
     ...
     */
    public RichTextRun getRun(int anIndex)
    {
        return _runs.get(anIndex);
    }
    ...
    private RichTextRun addRun(TextStyle theStyle, int anIndex)
    {
        ...
        if (theStyle != null) run.setStyle(theStyle);
        addRun(run, anIndex);
        return run;
    }
    ...
    {
        return getRunCount() > 0 ? getRun(getRunCount() - 1) : null;
    }
    ...
    protected void join(RichTextLine aLine)
    {
        for (int i = 0, iMax = aLine.getRunCount(); i < iMax; i++) {
            RichTextRun run = aLine.getRun(i), run2 = run.clone();
    ...
    public double getWidth()
    {
        ...
        _width = 0;
        for (RichTextRun run : _runs) _width += run.getWidth();
        return _width;
    }
    ...
    {
        RichTextLine clone = null;
        try {
            clone = (RichTextLine) super.clone();
        } catch (Exception e) {
